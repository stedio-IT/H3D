...
namespace Simple3DApp.Services
{
    ...
    public static class GCodeAsyncLoader
    {
        ...
        public class TubesMergedResult
        {
            public System.Collections.Generic.Dictionary<int, System.Collections.Generic.List<Simple3DApp.Models.SceneItem>> LayerToItems { get; } = new System.Collections.Generic.Dictionary<int, System.Collections.Generic.List<Simple3DApp.Models.SceneItem>>();
            public int MaxLayer { get; set; }
        }

        public static async System.Threading.Tasks.Task<TubesMergedResult> BuildTubesMergedAsync(
            string path, GCodeColorMode mode, int speedBins, float radius, int thetaDiv,
            System.IProgress<GCodeProgress> progress, System.Threading.CancellationToken token)
        {
            return await System.Threading.Tasks.Task.Run(() =>
            {
                var result = new TubesMergedResult();
                var pg = new GCodeProgress { TotalBytes = new System.IO.FileInfo(path).Length, Pass = 2 };

                // MeshBuilder per bucket (layer+tag)
                var builders = new System.Collections.Generic.Dictionary<(int layer, string tag), HelixToolkit.Wpf.SharpDX.MeshBuilder>();
                var materials = new System.Collections.Generic.Dictionary<string, HelixToolkit.Wpf.SharpDX.PhongMaterial>();

                long segCount = 0;
                using (var fs = System.IO.File.OpenRead(path))
                using (var sr = new System.IO.StreamReader(fs, System.Text.Encoding.ASCII, true, 1<<16))
                {
                    bool absXYZ = true, absE = true;
                    float x=0,y=0,z=0,e=0,f=0;
                    int layer = 0;
                    string line;
                    float fMin=0, fMax=0;
                    ...
                    if (mode == GCodeColorMode.BySpeed)
                    {
                        ...
                        using (var sr2 = new System.IO.StreamReader(fs2, System.Text.Encoding.ASCII, true, 1<<16))
                        {
                            string l2; float x2=0,y2=0,z2=0,e2=0,f2=0; bool a1=true,a2=true;
                            fMin = float.MaxValue; fMax = float.MinValue;
                            while ((l2 = sr2.ReadLine()) != null)
                            {
                                ParseModal(l2, ref a1, ref a2);
                                if (TryParseMove(l2, a1, a2, ref x2, ref y2, ref z2, ref e2, ref f2, out var nx, out var ny, out var nz, out var ne, out var nf, out var isMove))
                                {
                                    if (isMove)
                                    {
                                        var eff = nf!=0 ? nf : f2;
                                        if (eff>0) { if (eff<fMin) fMin=eff; if (eff>fMax) fMax=eff; }
                                        x2=nx; y2=ny; z2=nz; e2=ne; f2 = nf!=0 ? nf : f2;
                                    }
                                }
                            }
                        }
                    ...
                    while ((line = sr.ReadLine()) != null)
                    {
                        ...
                        ParseModal(line, ref absXYZ, ref absE);

                        if (TryParseLayerChange(line, ref layer)) { 
                            pg.BytesRead = bytes; pg.LayersParsed = layer; pg.SegmentsParsed = segCount; progress?.Report(pg);
                            continue; 
                        }

                        if (TryParseMove(line, absXYZ, absE, ref x, ref y, ref z, ref e, ref f, out var nx, out var ny, out var nz, out var ne, out var nf, out var isMove))
                        {
                            bool isExtrude = ne > e && (nx!=x || ny!=y || nz!=z);
                            bool isTravel = !isExtrude && (nx!=x || ny!=y || nz!=z);
                            if (isExtrude || isTravel)
                            {
                                string tag;
                                var col = new SharpDX.Color4(0.2f,0.8f,1f,1f);
                                if (mode == GCodeColorMode.ByType)
                                {
                                    if (isExtrude) { tag="EXTRUDE"; col = new SharpDX.Color4(1f,0.5f,0.1f,1f); }
                                    else { tag="TRAVEL"; col = new SharpDX.Color4(0.3f,0.3f,0.3f,0.6f); }
                                }
                                else if (mode == GCodeColorMode.BySpeed)
                                {
                                    float effF = nf!=0 ? nf : f;
                                    if (fMax <= fMin) effF = fMin;
                                    int bin = System.Math.Max(0, System.Math.Min(speedBins-1, (int)System.Math.Floor((effF - fMin) / System.Math.Max(1e-6f, (fMax - fMin)) * speedBins)));
                                    tag = $"SPEED_{bin}";
                                    col = ColorFromHSV(240f - 240f * (bin / (float)System.Math.Max(1, speedBins - 1)), 0.9f, 0.95f);
                                }
                                else
                                {
                                    tag = isExtrude ? "EXTRUDE" : "TRAVEL";
                                    col = isExtrude ? new SharpDX.Color4(0.1f,0.9f,0.2f,1f) : new SharpDX.Color4(0.3f,0.3f,0.3f,0.6f);
                                }

                                var key = (layer, tag);
                                if (!builders.TryGetValue(key, out var mb))
                                {
                                    mb = new HelixToolkit.Wpf.SharpDX.MeshBuilder(true, true, true);
                                    builders[key] = mb;

                                    // store material per tag
                                    if (!materials.ContainsKey(tag))
                                    {
                                        materials[tag] = new HelixToolkit.Wpf.SharpDX.PhongMaterial { DiffuseColor = col, AmbientColor = new SharpDX.Color4(0.1f,0.1f,0.1f,1f) };
                                    }
                                }

                                var s = new SharpDX.Vector3(x,y,z);
                                var t = new SharpDX.Vector3(nx,ny,nz);
                                if ((t - s).LengthSquared() > 1e-12f)
                                {
                                    mb.AddCylinder(s, t, radius, thetaDiv, true, true);
                                    segCount++;
                                }
                            }
                            x=nx; y=ny; z=nz; e=ne; f = nf!=0 ? nf : f;
                ...
                // Emit merged meshes per bucket
                foreach (var kv in builders)
                {
                    var layer = kv.Key.layer;
                    var tag = kv.Key.tag;
                    var mb = kv.Value;
                    var mesh = mb.ToMeshGeometry3D();
                    if (!result.LayerToItems.TryGetValue(layer, out var list))
                    {
                        list = new System.Collections.Generic.List<Simple3DApp.Models.SceneItem>();
                        result.LayerToItems[layer] = list;
                    }
                    list.Add(new Simple3DApp.Models.SceneItem
                    {
                        Name = tag,
                        Geometry = mesh,
                        Material = materials.TryGetValue(tag, out var mat) ? (HelixToolkit.Wpf.SharpDX.Material)mat : HelixToolkit.Wpf.SharpDX.PhongMaterials.Gray,
                        Transform = System.Windows.Media.Media3D.Transform3D.Identity
                    });
                }

                var maxL = 0;
                foreach (var l in result.LayerToItems.Keys) if (l > maxL) maxL = l;
                result.MaxLayer = maxL;
                return result;
            });
        }
